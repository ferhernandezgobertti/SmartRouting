...
"""
    PreDynamicApproach <: AbstractClassificationAlgorithm
...
customized predictive cost functions and geographical information from previously
established zones relative to the training routes with adaptavely selected attributes.
...
Outputs: routes, zclassif, zscores, fig_arr
* routes: List of updated routes (i.e. new stops associated to identified zones).
...
struct PreDynamicApproach <: AbstractClassificationAlgorithm end
...
ZonesAnalysis() = ZonesAnalysis("", 0.0, 0.0, 0.0, 0.0)
...
function calculate_cost_function(zanalisis, params)
    cost_value = 0
    if(params[11]==1)
        cost_value = params[12]*zanalisis.polygon_count^2+params[13]*zanalisis.internal_count+params[14]*min_dist_sides^2+params[15]*min_dist_vertex
    elseif(params[11]==2)
        cost_value = params[12]*zanalisis.polygon_count+params[13]*zanalisis.internal_count^2+params[14]*min_dist_sides+params[15]*min_dist_vertex^2
    else
        cost_value = params[12]*zanalisis.polygon_count+params[13]*zanalisis.internal_count+params[14]*min_dist_sides+params[15]*min_dist_vertex
    end
    return cost_value
...
function get_predicted_actual(routes, zones_cluster, params)
    ...
    for R in routes
        dyno_params = get_dynamic_selection(R, params)
        ...
        for S in R.stops
            ...
            if(zone(S)!="0")
                city_exists = false
                stop_polygon_max = 0
                min_dist_sides_min = dyno_params[4]
                min_dist_vertex_min = dyno_params[5]
                zone_pred_max = ""
                ...
                for ZC in zones_cluster
                    ...
                    if(stop_city==ZC.city)
                        ...
                        zanalisis = predict_zone_with_tolerance2(S, ZC, dyno_params)
                        ...
                        zanalisis.polygon_count >= dyno_params[3] && stop_polygon_count = zanalisis.polygon_count 
                        push!(zdetails.poly_count_arr, stop_polygon_count)
                        is_appropiate_cost_value = calculate_cost_function(zanalisis, dyno_params) >= dyno_params[16]
                        if( min_dist_vertex_min>=zanalisis.min_dist_vertex && is_appropiate_cost_value)
                            min_dist_vertex_min = zanalisis.min_dist_vertex
                        end
                        if(zanalisis.zone_pred!="" && zanalisis.min_dist_vertex==0 && is_appropiate_cost_value)
                            zone_pred_max = zanalisis.zone_pred
                            min_dist_vertex_min = zanalisis.min_dist_vertex
                            break
                        end
                        if(min_dist_sides_min>=zanalisis.min_dist_sides && is_appropiate_cost_value)
                            min_dist_sides_min = zanalisis.min_dist_sides
                        end
                        if(zanalisis.zone_pred!="" && zanalisis.min_dist_sides==0 && is_appropiate_cost_value)
                            zone_pred_max = zanalisis.zone_pred
                            min_dist_sides_min = zanalisis.min_dist_sides
                            break
                        end
                        if(zanalisis.zone_pred!="" && stop_polygon_count>=stop_polygon_max && is_appropiate_cost_value)
                            zone_pred_max = zanalisis.zone_pred
                ...
                if(zone_pred_max!=zone(S))
                    ...
                    min_dist_sides_act = dyno_params[4]
                    min_dist_vertex_act = dyno_params[5]
...
function get_adaptative_selection(route, params)
    ...
    route_diag = sqrt(route_length^2 + route_width^2)
    push!(params, round(0.4*rel_route,digits=3)) #zone_quant_param = dParams[12] # {1.0, 1.2, 1.5}
    push!(params, round(0.7*rel_route,digits=3)) #zone_internal_param = dParams[13] # {0, 1, 2, 3}
    push!(params, round(10.4*route_diag,digits=3)) #closeness_sides = dParams[14] # {0, 0.5, 1.2, 1.5}
    push!(params, round(9.8*route_diag,digits=3)) #closeness_vertex = dParams[15] # {0, 0.5, 1.2, 1.5}
    push!(params, round(1.2*rel_route,digits=3)) #compareVal = dParams[16] # {1.2, 1.4, 1.7}
    return params
end

function get_dynamic_selection(route, params)
    if(stops_in_region(route.stops,[30.134, 30.517],[-97.951, -97.531]))      # Austin
        insert!(params, 2, 3) #min_polygon_count = dParams[3] # {1, 2, 3}
        insert!(params, 0.0004, 4) #min_dist_sides = dParams[4] # {0.0002, 0.0004, 0.0005}
        insert!(params, 0.0007, 5) #min_dist_vertex = dParams[5] # {0.0007, 0.0010, 0.00014}
        insert!(params, 0.00005, 6) #tolerance = dParams[6] # {0.00001, 0.00003, 0.00005}
        insert!(params, 0.004, 7) #dif_limit_up = dParams[7] # {0.004, 0.005, 0.006}
        insert!(params, 0.002, 8) #dif_limit_down = dParams[8] # # {0.001, 0.002, 0.003}
        insert!(params, 2, 9) #distance_measure = dParams[9] # {1, 2} = {Manhattan, Euclidian}
        insert!(params, 3, 11) #cost_function = dParams[11] # {1, 2, 3} = {Quadratic V1, Quadratic V2, Lineal (Default)}
    elseif(stops_in_region(route.stops,[42.235, 42.397],[-71.199, -70.967]))  # Boston
        insert!(params, 3, 3) #min_polygon_count 
        insert!(params, 0.0002, 4) #min_dist_sides 
        insert!(params, 0.0007, 5) #min_dist_vertex
        insert!(params, 0.00001, 6) #tolerance
        insert!(params, 0.004, 7) #dif_limit_up
        insert!(params, 0.001, 8) #dif_limit_down
        insert!(params, 2, 9) #distance_measure
        insert!(params, 1, 11) #cost_function
    elseif(stops_in_region(route.stops,[41.653, 42.015],[-87.866, -87.487]))  # Chicago
        insert!(params, 2, 3) #min_polygon_count
        insert!(params, 0.0005, 4) #min_dist_sides
        insert!(params, 0.0010, 5) #min_dist_vertex
        insert!(params, 0.00003, 6) #tolerance
        insert!(params, 0.005, 7) #dif_limit_up
        insert!(params, 0.003, 8) #dif_limit_down
        insert!(params, 2, 9) #distance_measure
        insert!(params, 3, 11) #cost_function
    elseif(stops_in_region(route.stops,[33.748, 34.312],[-118.639, -118.152]))  # Los Angeles
        insert!(params, 1, 3) #min_polygon_count
        insert!(params, 0.0005, 4) #min_dist_sides
        insert!(params, 0.0014, 5) #min_dist_vertex
        insert!(params, 0.00003, 6) #tolerance
        insert!(params, 0.006, 7) #dif_limit_up
        insert!(params, 0.003, 8) #dif_limit_down
        insert!(params, 2, 9) #distance_measure
        insert!(params, 2, 11) #cost_function
    elseif(stops_in_region(route.stops,[47.502, 47.732],[-122.435, -122.244]))  # Seattle
        ...
        insert!(params, 0.005, 7) #dif_limit_up
        insert!(params, 0.001, 8) #dif_limit_down
        insert!(params, 1, 9) #distance_measure
        insert!(params, 2, 11) #cost_function
    else                                                                        # Montevideo
        insert!(params, 3, 3) #min_polygon_count
        insert!(params, 0.0002, 4) #min_dist_sides
    ...
    params = get_adaptative_selection(route, params)
    return params
end

function solve(alg::PreDynamicApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    #route_quantity = dParams[1] # {0, 10, 20, 50} = {All, N Routes}
    #max_zones = dParams[2] # {100, 150, 200}
    #dynamic_attributes = dParams[3:9]
    #update_valroutes = dParams[10] # {0, 1} = {True, False}
    #adaptative_attributes = dParams[11:16]
    dParams = [20, 150, 1]
    zclust = fill_zones_cluster(trainroutes)
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::PreDynamicApproach, valroutes::Vector{Route}, zclust::ZonesCluster)
    dParams = [20, 150, 1]
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::PreDynamicApproach, routes::Vector{Route}, zones_cluster::ZonesCluster; params)

    if(params[1]==0 && params[10]==0])
        _, zones_classif = get_predicted_actual(routes, zones_cluster, params)
    elseif(params[1]!=0 && params[10]==0)
        _, zones_classif = get_predicted_actual(routes[1:params[1]], zones_cluster, params)
    elseif(params[1]==0 && params[10]!=0)
        routes, zones_classif = get_predicted_actual(routes, zones_cluster, params)
    else
        routes, zones_classif = get_predicted_actual(routes[1:params[1]], zones_cluster, params)
    end
    zscores, fig_arr = get_score_prediction(zones_classif, zones_cluster, params)

    return routes, zclassif, zscores, fig_arr
end
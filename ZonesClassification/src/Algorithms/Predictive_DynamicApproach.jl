...
established zones relative to the training routes with dynamically selected attributes.
...
function get_dynamic_selection(route, params)
    ...
    elseif(stops_in_region(route.stops,[41.653, 42.015],[-87.866, -87.487]))  # Chicago
        ...
        insert!(params, 2, 9) #distance_measure = dParams[9]
...
function solve(alg::PreDynamicApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    #cost_function = dParams[11] # {1, 2, 3} = {Quadratic V1, Quadratic V2, Lineal (Default)}
    #zone_quant_param = dParams[12] # {1.0, 1.2, 1.5}
    #zone_internal_param = dParams[13] # {0, 1, 2, 3}
    #closeness_sides = dParams[14] # {0, 0.5, 1.2, 1.5}
    #closeness_vertex = dParams[15] # {0, 0.5, 1.2, 1.5}
    #compareVal = dParams[16] # {1.2, 1.4, 1.7}
    dParams = [20, 150, 1, 3, 1.2, 2, 1.5, 1.2, 1.4]

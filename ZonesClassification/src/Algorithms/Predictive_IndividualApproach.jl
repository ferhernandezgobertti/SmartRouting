...
"""
    PreIndividualApproach <: AbstractClassificationAlgorithm
...
established zones relative to the training routes, distinguishing per city.
...
struct PreIndividualApproach <: AbstractClassificationAlgorithm end
...
function solve(alg::PreIndividualApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    #selected_city = dParams[17] # {1, 2, 3, 4, 5, 6} = {Austin, Boston, Chicago, Los Angeles, Seattle, Montevideo}
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 3, 1.2, 2, 1.5, 1.2, 1.4, 6]
    zclust = fill_zones_cluster(trainroutes)
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::PreIndividualApproach, valroutes::Vector{Route}, zclust::ZonesCluster)
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 3, 1.2, 2, 1.5, 1.2, 1.4, 6]
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::PreIndividualApproach, routes::Vector{Route}, zones_cluster::ZonesCluster; params)
    filtered_routes = filter_routes(routes, params[17])

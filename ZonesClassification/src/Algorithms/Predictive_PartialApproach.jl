...
"""
    PrePartialApproach <: AbstractClassificationAlgorithm
...
established zones relative to the training routes. With partial attributes selection.
...
struct PrePartialApproach <: AbstractClassificationAlgorithm end
...
function is_stop_in_polygon_with_tolerance(stop, vertex, tolerance)
...
function predict_zone_with_tolerance(stop, zone_cluster, params)
    ...
    for i in 1:length(zone_cluster.polygon_arr)
        ...
        stop_internal_cant, length_cant, min_dist_side = is_stop_in_polygon_with_tolerance(stop, poly_vertices, params[6])
    ...
    params[17] == 1 && zanalisis.polygon_count = length(findall(x->x==1,zone_polygon_bool))
    params[18] == 1 && zanalisis.min_dist_sides = minimum(abs.(zone_polygon_arr))
    params[19] == 1 && zanalisis.min_dist_vertex = minimum(abs.(zone_vertices_arr))
    params[20] == 1 && zanalisis.internal_count = maximum(stop_internal_arr)
    return zanalysis
end
            ...
            if(zone(S)!="0")
                ...
                for ZC in zones_cluster
                    ...
                    if(stop_city==ZC.city)
                        city_exists = true
                        #zone_pred, polygon_count, min_dist_sides, min_dist_vertex, internal_count = predict_zone_with_tolerance(S, ZC, params)
                        zanalisis = predict_zone_with_tolerance(S, ZC, params)
                        ...
                        is_appropiate_cost_value = calculate_cost_function(zanalisis, params) >= params[16]
...
function solve(alg::PrePartialApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    #disable_polygon_count = dParams[17] # {0, 1} = {True, False}
    #disable_min_point_dist = dParams[18] # {0, 1} = {True, False}
    #disable_min_vertex_dist = dParams[19] # {0, 1} = {True, False}
    #disable_internal_count = dParams[20] # {0, 1} = {True, False}
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 3, 1.2, 2, 1.5, 1.2, 1.4, 1, 0, 0, 1]
    zclust = fill_zones_cluster(trainroutes)
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::PrePartialApproach, valroutes::Vector{Route}, zclust::ZonesCluster)
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 3, 1.2, 2, 1.5, 1.2, 1.4]
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::PrePartialApproach, routes::Vector{Route}, zones_cluster::ZonesCluster; params)

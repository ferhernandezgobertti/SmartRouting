...
"""
    PreRandomApproach <: AbstractClassificationAlgorithm
...
struct PreRandomApproach <: AbstractClassificationAlgorithm end
...
function get_random_routes_selection(routes, params)
    ...
    Random.seed!(params[18])
    rng = MersenneTwister(params[18])
    if(params[19] == 1)
        perm_seg = randcycle(rng, routes_quantity)
        train_seg = perm_seg[1:params[17]]
        val_seg =  perm_seg[params[17]+1:routes_quantity]
    elseif(params[19] == 2)
...
function solve(alg::PreRandomApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    #train_routes_quantity = dParams[17] # {N < Total}
    #random_seed = dParams[18] # {1000, 2500, 4000}
    #random_distribution = dParams[19] # {1, 2, 3, 4} = {RandCycle, RandPerm, RandSubSeq, Shuffle}
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 3, 1.2, 2, 1.5, 1.2, 1.4, 6000, 4000, 4]
    return solve(alg, routes; params=dParams)
end

function solve(alg::PreRandomApproach, routes::Vector{Route}; params)

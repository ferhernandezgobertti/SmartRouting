...
"""
    ProIndividualApproach <: AbstractClassificationAlgorithm
...
struct ProIndividualApproach <: AbstractClassificationAlgorithm end
...
function solve(alg::ProIndividualApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    #selected_city = dParams[11] # {1, 2, 3, 4, 5, 6} = {Austin, Boston, Chicago, Los Angeles, Seattle, Montevideo}
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 6]
    zclust = fill_zones_cluster(trainroutes)
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::ProIndividualApproach, valroutes::Vector{Route}, zclust::ZonesCluster)
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 6]
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::ProIndividualApproach, routes::Vector{Route}, zones_cluster::ZonesCluster; params)
    filtered_routes = filter_routes(routes, params[11])
    if(params[1]==0 && params[10]==0])
        _, zones_classif = get_predicted_actual(filtered_routes, zones_cluster, params)
    elseif(params[1]!=0 && params[10]==0)
        _, zones_classif = get_predicted_actual(filtered_routes[1:params[1]], zones_cluster, params)
    elseif(params[1]==0 && params[10]!=0)
        filtered_routes, zones_classif = get_predicted_actual(filtered_routes, zones_cluster, params)
    else
        filtered_routes, zones_classif = get_predicted_actual(filtered_routes[1:params[1]], zones_cluster, params)
    end
    zscores, fig_arr = get_score_prediction(zones_classif, zones_cluster, params)

    return filtered_routes, zclassif, zscores, fig_arr
end
...
"""
    ProPartialApproach <: AbstractClassificationAlgorithm
...
struct ProPartialApproach <: AbstractClassificationAlgorithm end
...
function predict_zone_with_tolerance(stop, zone_cluster, params)
    ...
    params[11] == 1 && zanalisis.polygon_count = length(findall(x->x==1,zone_polygon_bool))
    params[12] == 1 && zanalisis.min_dist_sides = minimum(abs.(zone_polygon_arr))
    params[13] == 1 && zanalisis.min_dist_vertex = minimum(abs.(zone_vertices_arr))
    params[14] == 1 && zanalisis.internal_count = maximum(stop_internal_arr)
    return zanalysis
end
...
function solve(alg::ProPartialApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    #disable_polygon_count = dParams[11] # {0, 1} = {True, False}
    #disable_min_point_dist = dParams[12] # {0, 1} = {True, False}
    #disable_min_vertex_dist = dParams[13] # {0, 1} = {True, False}
    #disable_internal_count = dParams[14] # {0, 1} = {True, False}
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1, 1, 0, 0, 1]
    zclust = fill_zones_cluster(trainroutes)
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::ProPartialApproach, valroutes::Vector{Route}, zclust::ZonesCluster)
    dParams = [20, 150, 1, 0.0004, 0.0007, 0.00005, 0.004, 0.001, 2, 1]
    return solve(alg, valroutes, zclust; params=dParams)
end

function solve(alg::ProPartialApproach, routes::Vector{Route}, zones_cluster::ZonesCluster; params)

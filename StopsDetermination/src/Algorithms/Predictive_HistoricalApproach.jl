"""
    HistoricalApproach <: AbstractDeterminationAlgorithm

Algorithm that determines the stops from raw GPS registries while looking historically 
...
struct HistoricalApproach <: AbstractDeterminationAlgorithm end
...
# Delta Spatial Difference Value
function get_position_delta(latc, lonc)
    lat_measure = sum([abs(latc[i]-latc[i+1]) for i in 1:length(latc)])
    lon_measure = sum([abs(lonc[i]-lonc[i+1]) for i in 1:length(lonc)])
    #println("POS DIF: "*string(abs(latc[1]-latc[2]))*" & "*string(abs(lonc[1]-lonc[2])))
    return sqrt(lat_measure^2+lon_measure^2)
end
...
# Delta Temporal Difference Value
function get_time_delta(timec, datec, is_time_american)
    return sum([get_duration(timec[i:i+1], datec[i:i+1], is_time_american) for i in 1:length(timec)])
end

# Delta Velocity Difference Value
function get_vel_delta(velc)
    return sum([velc[i] - velc[i+1] for i in 1:length(velc)])
end

# Delta Velocity Difference Value
function get_km_delta(kmc)
    return sum([kmc[i] - kmc[i+1] for i in 1:length(kmc)])
end
...
function distinguish_stops(transport, auxiliary, first_values, is_time_american, dParams)
    ...
    for i in [dParams[1]:1:length(transport.time_camion);]
        datec = get_backward_values(transport.date_camion, i, dParams[1]) #[transport.date_camion[i], first_values[1]]
        timec = get_backward_values(transport.time_camion, i, dParams[1]) #[transport.time_camion[i], first_values[2]]
        latc = get_backward_values(transport.lat_camion, i, dParams[1]) #[transport.lat_camion[i], first_values[3]]
        lonc = get_backward_values(transport.lon_camion, i, dParams[1]) #[transport.lon_camion[i], first_values[4]]
        velc = get_backward_values(transport.vel_camion, i, dParams[1])
        kmc = get_backward_values(transport.km_camion, i, dParams[1]) #[transport.km_camion[i], first_values[5]]

        alpha_position = dParams[5]*get_position_delta(latc, lonc)
        alpha_time = dParams[4]*get_time_delta(timec, datec, is_time_american)
        alpha_vel = dParams[3]*get_vel_delta(velc)
        alpha_km = dParams[2]*get_km_delta(kmc)

        cost_value = alpha_position + alpha_time + alpha_vel + alpha_km

        if(cost_value < dParams[6])
            stops_quantity += 1
        else
            if(stops_quantity>=dParams[1])
...
function solve(alg::HistoricalApproach, filenames::Vector{String}, is_time_american::Bool)

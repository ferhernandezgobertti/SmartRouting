"""
    GuidedApproach <: AbstractDeterminationAlgorithm

Algorithm that determines the stops from raw GPS registries while looking bidirectionally 
...
struct GuidedApproach <: AbstractDeterminationAlgorithm end
...
function is_position_close(latcB, loncB, latcF, loncF, close_dist_measure)
    is_lat_close = sum([abs(latcB[i]-latcB[i+1]) for i in 1:length(latcB)]) + sum([abs(latcF[i]-latcF[i+1]) for i in 1:length(latcF)])<=close_dist_measure
    is_lon_close = sum([abs(loncB[i]-loncB[i+1]) for i in 1:length(loncB)]) + sum([abs(loncF[i]-loncF[i+1]) for i in 1:length(loncF)])<=close_dist_measure
...
function is_time_appropiate(timecB, datecB, timecF, datecF, close_time_measure, is_time_american)
    return sum([get_duration(timecB[i:i+1], datecB[i:i+1], is_time_american) for i in 1:length(timecB)]) + sum([get_duration(timecF[i:i+1], datecF[i:i+1], is_time_american) for i in 1:length(timecF)]) >= close_time_measure
end

# Checking if contiguous registries are velocity appropiate
function is_vel_appropiate(velcB, velcF, stop_vel_measure)
    return sum([velcB[i] - velcB[i+1] for i in 1:length(velcB)]) + sum([velcF[i] - velcF[i+1] for i in 1:length(velcF)])<= stop_vel_measure
end

# Checking if contiguous registries are kilometer appropiate
function is_km_appropiate(kmcB, kmcF, stop_km_measure)
    return sum([kmcB[i] - kmcB[i+1] for i in 1:length(kmcB)]) + sum([kmcF[i] - kmcF[i+1] for i in 1:length(kmcF)]) <= stop_km_measure
end
...
# Backward Data Subgroup Vector
function get_backward_values(orig_vector, init_pos, first_value)
    return orig_vector[i-first_value:1:i]
end
...
function distinguish_stops(transport, auxiliary, first_values, is_time_american, dParams)
    ...
    #for i in [length(transport.time_camion):-1:dParams[1];]
    for i in [dParams[1]:1:length(transport.time_camion)-dParams[1];]
        datecB = get_backward_values(transport.date_camion, i, dParams[1]) #[transport.date_camion[i], first_values[1]]
        timecB = get_backward_values(transport.time_camion, i, dParams[1]) #[transport.time_camion[i], first_values[2]]
        latcB = get_backward_values(transport.lat_camion, i, dParams[1]) #[transport.lat_camion[i], first_values[3]]
        loncB = get_backward_values(transport.lon_camion, i, dParams[1]) #[transport.lon_camion[i], first_values[4]]
        velcB = get_backward_values(transport.vel_camion, i, dParams[1])
        kmcB = get_backward_values(transport.km_camion, i, dParams[1]) #[transport.km_camion[i], first_values[5]]
        datecF = get_forward_values(transport.date_camion, i, dParams[1]) #[transport.date_camion[i], first_values[1]]
        timecF = get_forward_values(transport.time_camion, i, dParams[1]) #[transport.time_camion[i], first_values[2]]
        latcF = get_forward_values(transport.lat_camion, i, dParams[1]) #[transport.lat_camion[i], first_values[3]]
        loncF = get_forward_values(transport.lon_camion, i, dParams[1]) #[transport.lon_camion[i], first_values[4]]
        velcF = get_forward_values(transport.vel_camion, i, dParams[1])
        kmcF = get_forward_values(transport.km_camion, i, dParams[1]) #[transport.km_camion[i], first_values[5]]

        is_close_position = is_position_close(latcB, loncB, latcF, loncF, dParams[5])
        is_appropiate_time = is_time_appropiate(timecB, datecB, timecF, datecF, dParams[4], is_time_american)
        is_appropriate_vel = is_vel_appropiate(velcB, velcF, dParams[3])
        is_appropiate_km = is_km_appropiate(kmcB, kmcF, dParams[2])
...
function solve(alg::GuidedApproach, filenames::Vector{String}, is_time_american::Bool)

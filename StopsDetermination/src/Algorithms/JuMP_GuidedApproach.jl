"""
    GuidedApproachJuMP <: AbstractDeterminationAlgorithm
...
struct GuidedApproachJuMP <: AbstractDeterminationAlgorithm end
...
# Model Definition and Application of GuidedApproach (Bidirectional) for Stops Determination
function applicate_model_forward(transport, dParams; solver=OptSolver)
    ...
    for i in dParams[1]:n
        ...
        # Distance Constraint
        constraint_distance_forw = sum([sqrt((transport.lat_camion[i+j-1]*x[i+j-1]-transport.lat_camion[i+j]*x[i+j])^2 + (transport.lon_camion[i+j-1]*x[i+j-1]-transport.lon_camion[i+j]*x[i+j])^2) for j in 1:dParams[1]])
        constraint_distance_back = sum([sqrt((transport.lat_camion[i-j-1]*x[i-j-1]-transport.lat_camion[i-j]*x[i-j])^2 + (transport.lon_camion[i-j-1]*x[i-j-1]-transport.lon_camion[i-j]*x[i-j])^2) for j in 1:dParams[1]])
        @constraint(model, constraint_distance_forw + constraint_distance_back < dParams[5])
        # Temporal Constraint
        constraint_temporal_forw = sum([get_duration(transport.time_camion[i:i+1]*x[i], transport.date_camion[i:i+1]*x[i], is_time_american) for i in 1:length(transport.time_camion)])
        constraint_temporal_back = sum([get_duration(transport.time_camion[i-1:i]*x[i], transport.date_camion[i-1:i]*x[i], is_time_american) for i in 1:length(transport.time_camion)])
        @constraint(model, constraint_temporal_forw + constraint_temporal_back < dParams[4])
        # Velocity Constraint
        constraint_velocity_forw = sum([transport.vel_camion[i+j-1]*x[i+j-1]-transport.vel_camion[i+j]*x[i+j] for j in 1:dParams[1]])
        constraint_velocity_back = sum([transport.vel_camion[i-j-1]*x[i-j-1]-transport.vel_camion[i-j]*x[i-j] for j in 1:dParams[1]])
        @constraint(model, constraint_velocity_forw + constraint_velocity_back < dParams[3])
        # Kilometer Constraint
        constraint_kilometer_forw = sum([transport.km_camion[i+j-1]*x[i+j-1]-transport.km_camion[i+j]*x[i+j] for j in 1:dParams[1]])
        constraint_kilometer_back = sum([transport.km_camion[i-j-1]*x[i-j-1]-transport.km_camion[i-j]*x[i-j] for j in 1:dParams[1]])
        @constraint(model, constraint_kilometer_forw + constraint_kilometer_back < dParams[2]) 
    end
...
function solve(alg::GuidedApproachJuMP, filenames::Vector{String}, is_time_american::Bool)

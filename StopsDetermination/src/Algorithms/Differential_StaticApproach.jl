"""
    StaticApproach <: AbstractDeterminationAlgorithm

Algorithm that determines the stops from raw GPS registries while looking differentially 
on every static contiguous position in the chronologically order measurements.
...
struct StaticApproach <: AbstractDeterminationAlgorithm end
...
function is_position_close(latc, lonc, close_dist_measure)
    is_lat_close = abs(latc[1]-latc[2])<=close_dist_measure
    is_lon_close = abs(lonc[1]-lonc[2])<=close_dist_measure
...
function is_currently_stopped(event_camion)
    is_stopped = true
...
function distinguish_stops(transport, auxiliary, first_values, is_time_american, dParams)
    ...
    for i in [2:1:length(transport.time_camion)-dParams[1];]
        datec = [transport.date_camion[i], first_values[1]]
        timec = [transport.time_camion[i], first_values[2]]
        latc = [transport.lat_camion[i], first_values[3]]
        lonc = [transport.lon_camion[i], first_values[4]]
        kmc = [transport.km_camion[i], first_values[5]]
        ...
        is_appropriate_vel = is_vel_appropiate(transport.vel_camion[i], dParams[3])
        is_appropiate_km = ((kmc[1] - kmc[2]) <= dParams[2])  

        is_stopped = is_currently_stopped(transport.event_camion)
        ...
        if(is_same_stop_registry(is_close_position, is_appropiate_time, is_appropriate_vel, is_appropiate_km, is_stopped))
            ...
            if(stops_quantity>=dParams[1] forfor (transport.event_camion[i]=="Encendido" && (encendido_pos-apagado_pos) in [1,2] && is_close_position && is_appropiate_time))
                push!(auxiliary.stops_lat, latc[2])
                push!(auxiliary.stops_lon, lonc[2])
                push!(auxiliary.measure_per_stop, stops_quantity)
                push!(auxiliary.kilometer_arr, kmc[1]-kmc[2])
                push!(auxiliary.stops_date, get_latin_date(first_date, is_time_american))
                push!(auxiliary.stops_time, first_values[2])
                push!(auxiliary.planned_time, get_duration(timec, datec, is_time_american))
            end
            stops_quantity = 0
...
function solve(alg::StaticApproach, filenames::Vector{String}, is_time_american::Bool)

"""
    BackwardApproachJuMP <: AbstractDeterminationAlgorithm
...
struct BackwardApproachJuMP <: AbstractDeterminationAlgorithm end
...
# Model Definition and Application of BackwardApproach (Reversal) for Stops Determination
function applicate_model_forward(transport, dParams; solver=OptSolver)
    ...
    for i in dParams[1]:n
        ...
        # Temporal Constraint
        @constraint(model, sum([get_duration(transport.time_camion[i-1:i]*x[i], transport.date_camion[i-1:i]*x[i], is_time_american) for i in 1:length(transport.time_camion)]) < dParams[4])
        # Velocity Constraint
...
function solve(alg::BackwardApproachJuMP, filenames::Vector{String}, is_time_american::Bool)

...
"""
    DynamicApproach <: AbstractIdentificationAlgorithm
...
identification rules dynamically configured per city distintion.
...
struct DynamicApproach <: AbstractIdentificationAlgorithm end
...
function solve(alg::DynamicApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    dParams = [2.2, 1.5, 1.7, 1.3, 0.614, 0.231, 0.245, 0.089]
...
function solve(alg::DynamicApproach, routes::Vector{Route}, zvar::ZonesVar; params=dParams)
    ...
    for R in routes
        ...
        ref_zone, zones_centers = get_ref_zone(R, dyno_params)
        ref_zone.ids = get_ref_identifier(R, ref_zone.center, dyno_params)
        ...
        for Z in zones(R)
            ...
            curr_zone.ids = manage_identifiers(zone_proj_mods, ref_zone, dyno_params)

...
"""
    IndividualApproach <: AbstractIdentificationAlgorithm
...
identification rules, enabling filtering distinction per individual city.
...
struct IndividualApproach <: AbstractIdentificationAlgorithm end
...
function stops_in_region(stops, lat_range, lng_range)
    #for S in stops
    #    if(latitude(S) < lat_range[1] || latitude(S) > lat_range[2])
    #        in_lat_region = false
    #        break
    #    end
    #end
...
function solve(alg::IndividualApproach, trainroutes::Vector{Route}, valroutes::Vector{Route})
    ...
    #selected_city = dParams[15] # {1, 2, 3, 4, 5, 6} = {Austin, Boston, Chicago, Los Angeles, Seattle, Montevideo}
    dParams = [2.2, 1.5, 1.7, 1.3, 0.614, 0.231, 0.245, 0.089, 5, -3, 4, 1, 2, 0, 5]
    #compareval = [14.3, 15.6, 12.3, 16.8]
    filtered_train_routes = filter_routes(trainroutes, dParams[15])
    zclust = fill_zones_cluster(filtered_train_routes)
    ...
    return solve(alg, valroutes, zvar; params=dParams)
end

function solve(alg::IndividualApproach, routes::Vector{Route}, zvar::ZonesVar; params=dParams)

    vector_variations = 1:4
    filtered_routes = filter_routes(routes, dParams[15])
    zones_arr = Vector{DZones}()
    zones_ref_arr = Vector{DZones}()
    
    for R in filtered_routes
        route_num = findfirst(x->name(x)=name(R), filtered_routes)
    ...
    return filtered_routes, zones_arr, zones_ref_arr
end
...
"""
MinTimeComb4 <: AbstractSequencingAlgorithm
...
cluster sequencing through set comparison, stops sequencing using local minimum cost, 
last inner stop as transition stop and route perspective’s time window as well as 
...
struct MinTimeComb4{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeComb4() = MinTimeComb4(MinTime())
...
function get_zones_hausdorff_matrix(R::AbstractRoute; tol=1e-4)
...
function get_zones_list(z_dist, initial_stop)
    z_list = Vector{String}()
    curr_zone = ZonesBar('0', [initial_stop.lat, initial_stop.lng])
    for ZD in z_dist
        dist_vector = [get_distance(z_dist[i].baricenter, curr_zone.baricenter) for i in 1:length(z_dist)]
        z_next = z_dist[findmin(dist_vector[findall(x->x!=0.0, dist_vector)])[2]].identifier
        push!(z_list, z_next)
        curr_zone = z_next
    end
    return z_list
...
function solve(alg::MinTimeComb4, valroute::AbstractRoute, costs::TravelTimes; initial_stop=get_station(route), params)
    T = costs.travel_times
    z_dist = get_zones_hausdorff_matrix(valroute)
    z_list = get_zones_list(z_dist, initial_stop)
...
function solve(alg::MinTimeComb4, valroute::AbstractRoute, costs::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
    T = costs.travel_times
    tw_stops = valroute.stops[findall(x->is_stop_with_time_window(x), valroute.stops)]
    [push!(sequence, tw_stops[i]) for i in 1:length(tw_stops)]
    [deleteat!(valroute.stops, findfirst(x->is_same_stop(x,tw_stops[i]), valroute.stops)) for i in 1:length(tw_stops)]
...
function is_stop_with_time_window(stop)
    for P in stop.packages
        is_with_tw = has_time_window(P)
        is_with_tw && break
    end
    return is_with_tw
end

function solve(alg::MinTimeComb4, route::AbstractRoute, times::TravelTimes; optimizer, initial_stop=station(route))
...
function solve(alg::MinTimeComb4, route::AbstractRoute, T::AbstractMatrix; zones_list=get_zones_order(route), initial_stop=get_station(route))
    route_id = name(route)
    algz = alg.algz
    ...
    while length(zones_list) > 0
        ...
        is_station(initial_stop) && final_inner_stop=initial_stop # Case first iteration
        subsequence, final_inner_stop = solve(algz, subroute, subtimes, initial_stop=final_inner_stop)
        subsequence = manage_capacity(subsequence, route.capacity, initial_stop)
        # append stops to the result
        for s in subsequence.stops
            push!(sequence, s)
        end
        final_inner_stop = sequence[length(sequence)]
        deleteat!(zones_list, idx)
    end
    return sequence, final_inner_stop
end

# simply return the first element of the list
function _next_stop(::MinTimeComb4, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeComb4(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score

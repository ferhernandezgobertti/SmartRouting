...
"""
MinTimeComb5 <: AbstractSequencingAlgorithm
...
last inner stop as transition stop and cluster perspective’s time window as well as 
...
struct MinTimeComb5{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeComb5() = MinTimeComb5(MinTime())
...
function solve(alg::MinTimeComb5, valroute::AbstractRoute, costs::TravelTimes; initial_stop=get_station(route), params)
...
function solve(alg::MinTimeComb5, valroute::AbstractRoute, costs::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
    T = costs.travel_times
...
function solve(alg::MinTimeComb5, route::AbstractRoute, times::TravelTimes; optimizer, initial_stop=station(route))
...
function solve(alg::MinTimeComb5, route::AbstractRoute, T::AbstractMatrix; zones_list=get_zones_order(route), initial_stop=get_station(route))
    route_id = name(route)
    algz = alg.algz
    ...
    [deleteat!(route.stops, findfirst(x->is_same_stop(x,tw_stops[i]), route.stops)) for i in 1:length(tw_stops)]
...
function _next_stop(::MinTimeComb5, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeComb5(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score

...
"""
MinTimeComb6 <: AbstractSequencingAlgorithm
...
struct MinTimeComb6{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeComb6() = MinTimeComb6(MinTime())
...
function solve(alg::MinTimeComb6, valroute::AbstractRoute, costs::TravelTimes; initial_stop=get_station(route), params)
...
function solve(alg::MinTimeComb6, valroute::AbstractRoute, costs::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
    T = costs.travel_times
...
function solve(alg::MinTimeComb6, route::AbstractRoute, times::TravelTimes; optimizer, initial_stop=station(route))
...
function manage_capacity(stops_sequence, max_capacity, initial_stop)
    ...
    for i in 3:length(stops_sequence)
        if(zone(stops_sequence[i]) != zone(stops_sequence[i-1]) && capacity >= max_capacity)
            insert!(stops_sequence, i, initial_stop)
            capacity = 0.0
        else
...
function solve(alg::MinTimeComb6, route::AbstractRoute, T::AbstractMatrix; zones_list=get_zones_order(route), initial_stop=get_station(route))
    route_id = name(route)
    algz = alg.algz
...
function _next_stop(::MinTimeComb6, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeComb6(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score

...
"""
MinTimeDirectIndividual <: AbstractSequencingAlgorithm
...
training route data with filtering capabilities, individually per selected city
...
struct MinTimeDirectIndividual{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeDirectIndividual() = MinTimeDirectIndividual(MinTime())
...
function regression_cluster_ordering(training_routes::Vector{Route}, training_sequences::Vector{Sequence}, validation_routes::Vector{Route})
    ...
    # selected_city = dParams[19] # {1, 2, 3, 4, 5, 6} = {Austin, Boston, Chicago, Los Angeles, Seattle, Montevideo}
    dParams = [0, 0, 1, 7, 4, 3, 3, 1, 3, 1, 7, 14, 9, 2, 10, 8, 4, 2, 4]
...
function regression_cluster_ordering(ord_training_routes::Vector{Route}, validation_routes::Vector{Route}; params)
    ...
    ord_training_routes = filter_routes(ord_training_routes, params[19])
    validation_routes = filter_routes(validation_routes, params[19])
...
function solve(alg::MinTimeDirectIndividual, valroute::AbstractRoute, times::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
...
function solve(alg::MinTimeDirectIndividual, route::AbstractRoute, T::AbstractMatrix; zones_list=get_zones_order(route), initial_stop=get_station(route))
    route_id = name(route)
    #T = times.travel_times
    algz = alg.algz
...
function _next_stop(::MinTimeDirectIndividual, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeDirectIndividual(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score

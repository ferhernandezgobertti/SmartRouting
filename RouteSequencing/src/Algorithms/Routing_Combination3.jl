...
"""
MinTimeComb3 <: AbstractSequencingAlgorithm
...
struct MinTimeComb3{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeComb3() = MinTimeComb3(MinTime())
...
function solve(alg::MinTimeComb3, valroute::AbstractRoute, costs::TravelTimes; initial_stop=get_station(route), params)
...
function solve(alg::MinTimeComb3, valroute::AbstractRoute, costs::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
    T = costs.travel_times
    tw_stops = valroute.stops[findall(x->is_stop_with_time_window(x), valroute.stops)]
    [push!(sequence, tw_stops[i]) for i in 1:length(tw_stops)]
    [deleteat!(valroute.stops, findfirst(x->is_same_stop(x,tw_stops[i]), valroute.stops)) for i in 1:length(tw_stops)]
...
function solve(alg::MinTimeComb3, route::AbstractRoute, times::TravelTimes; optimizer, initial_stop=station(route))
...
function solve(alg::MinTimeComb3, route::AbstractRoute, T::AbstractMatrix; zones_list=get_zones_order(route), initial_stop=get_station(route))
    route_id = name(route)
    algz = alg.algz
...
function _next_stop(::MinTimeComb3, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeComb3(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score

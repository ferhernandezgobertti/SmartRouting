...
"""
MinTimeComb1 <: AbstractSequencingAlgorithm

Algorithm that decides the stops sequence based on auxiliary minimization optimization
algorithm guided by complete zones sequencing modelling through one combination of 
cluster sequencing through metric distance, stops sequencing using global minimum cost, 
first outer stop as transition stop and route perspective’s time window as well as 
cluster capacity corroborations.
...
struct MinTimeComb1{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeComb1() = MinTimeComb1(MinTime())

struct ZonesBar
    identifier::String
    baricenter::Vector{Float64}
end
...
function get_coord_baricenter(z_stops)
    stops_ammount = length(z_stops)
    bar_latitude = sum([latitude(z_stops[i]) for i in 1:stops_ammount])/stops_ammount
    bar_longitude = sum([longitude(z_stops[i]) for i in 1:stops_ammount])/stops_ammount
    return [bar_latitude, bar_longitude]
end

function get_zones_baricenters(valroute)
    z_bars = Vector{ZonesBar}()
    z_valroute = zones(valroute)
    for Z in z_valroute
        z_stops = valroute.stops[findall(x->zone(x)==Z, valroute.stops)]
        z_bar = ZonesBar(Z, get_coord_baricenter(z_stops))
        push!(z_bars, z_bar)
    end
    return z_bars
end
...
function get_zones_list(z_bars, initial_stop)
    z_list = Vector{String}()
    curr_zone = ZonesBar('0', [initial_stop.lat, initial_stop.lng])
    for ZB in z_bars
        dist_vector = [get_distance(z_bars[i].baricenter, curr_zone.baricenter) for i in 1:length(z_bars)]
        z_next = z_bars[findmin(dist_vector[findall(x->x!=0.0, dist_vector)])[2]].identifier
        push!(z_list, z_next)
        curr_zone = z_next
    end
    return z_list
...
function is_same_stop(s1, s2)
    return s1.lat == s2.lat && s1.lng == s2.lng
end

function solve(alg::MinTimeComb1, valroute::AbstractRoute, costs::TravelTimes; initial_stop=get_station(route), params)
    T = costs.travel_times
    z_bars = get_zones_baricenters(valroute)
    z_list = get_zones_list(z_bars, initial_stop)
    tw_stops = valroute.stops[findall(x->is_stop_with_time_window(x), valroute.stops)]
    [push!(sequence, tw_stops[i]) for i in 1:length(tw_stops)]
    [deleteat!(valroute.stops, findfirst(x->is_same_stop(x,tw_stops[i]), valroute.stops)) for i in 1:length(tw_stops)]
    return solve(alg, valroute, T, zones_list=z_list, initial_stop=initial_stop)
end

function solve(alg::MinTimeComb1, valroute::AbstractRoute, costs::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
    T = costs.travel_times
    tw_stops = valroute.stops[findall(x->is_stop_with_time_window(x), valroute.stops)]
    [push!(sequence, tw_stops[i]) for i in 1:length(tw_stops)]
    [deleteat!(valroute.stops, findfirst(x->is_same_stop(x,tw_stops[i]), valroute.stops)) for i in 1:length(tw_stops)]
...
function is_stop_with_time_window(stop)
    is_with_tw = false
    for P in stop.packages
        is_with_tw = has_time_window(P) && break
    end
    return is_with_tw
end

function solve(alg::MinTimeComb1, route::AbstractRoute, times::TravelTimes; optimizer, initial_stop=station(route))
    T = times.travel_times
    return solve(MinTimeGlobal, route, T, optimizer=optimizer, initial_stop=initial_stop)
end

function get_capacity(p_dims)
    return p_dims.depth*p_dims.height*p_dims.width
end

function manage_capacity(stops_sequence, max_capacity, initial_stop)
    capacity = 0.0
    for i in 3:length(stops_sequence)
        if(zone(stops_sequence[i]) != zone(stops_sequence[i-1]) && capacity >= max_capacity)
            insert!(stops_sequence, 1, initial_stop)
            capacity = 0.0
        else
            capacity = capacity + sum([get_capacity(dimensions(stops_sequence[i].packages[j])) for j in 1:length(stops_sequence)])
        end
    end
    return stops_sequence
end

function solve(alg::MinTimeGlobal, route::AbstractRoute, T::AbstractMatrix; optimizer, initial_stop=station(route))
    S = stops(route)
    num_stops = length(S)
    route_id = name(route)
    seq = Vector{BareStop}()
    out = Sequence(route_id, seq)
    tw_stops = route.stops[findall(x->is_stop_with_time_window(x), route.stops)]
    [push!(sequence, tw_stops[i]) for i in 1:length(tw_stops)]
    if length(S) == 1
    ...
    result = manage_capacity(result, route.capacity, initial_stop)
    for r in result
...
function solve(alg::MinTimeComb1, route::AbstractRoute, T::AbstractMatrix; zones_list=get_zones_order(route), initial_stop=get_station(route))
    route_id = name(route)
    algz = alg.algz
    # zonas que quedan por recorrer
    idx = findfirst(x -> x == zone(initial_stop), zones_list)
    sequence = Sequence(route_id, Vector{BareStop}())
    while length(zones_list) > 0
        # filtrar solo las paradas de la zona actual
        current_zone = zones_list[idx]
        idx_stops = findall(s -> split(zone(s), ".")[1] == current_zone, route.stops)
        ...
        deleteat!(zones_list, idx)
    end
    return sequence
end

# simply return the first element of the list
function _next_stop(::MinTimeComb1, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeComb1(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score

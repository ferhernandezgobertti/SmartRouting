...
"""
MinTimeDirectDynamic <: AbstractSequencingAlgorithm
...
training route data with filtering capabilities, with dynamic attributes assignment.
...
struct MinTimeDirectDynamic{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeDirectDynamic() = MinTimeDirectDynamic(MinTime())
...
function solve(alg::MinTimeDirectDynamic, valroute::AbstractRoute, times::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
...
function solve(alg::MinTimeDirectDynamic, route::AbstractRoute, T::AbstractMatrix; zones_list, initial_stop=get_station(route))
    route_id = name(route)
    #T = times.travel_times
    algz = alg.algz
...
function _next_stop(::MinTimeDirectDynamic, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeDirectDynamic(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score

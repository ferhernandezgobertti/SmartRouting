...
"""
MinTimeDirectAdaptative <: AbstractSequencingAlgorithm
...
training route data with filtering capabilities, with adaptative attributes assignment.
...
struct MinTimeDirectAdaptative{T} <: AbstractSequencingAlgorithm
    algz::T
end

MinTimeDirectAdaptative() = MinTimeDirectAdaptative(MinTime())
...
function process_training_routes(ord_routes, params)
    ...
    for R in ord_routes
        params = get_dynamic_selection(R, params)
...
function get_zone_value(zone_id, adaptative_params)
    # alpha_param = dParams[3] {8, 10, 12} [15]
    # beta_param = dParams[4] {5, 6, 8} [16]
    # gamma_param = dParams[5] {2, 4, 5} [17]
    # delta_param = dParams[6] {0.8, 1, 2} [18]
    return adaptative_params[1]*zone_id[1:2] + adaptative_params[2]*split(zone_id[3:end],".")[1] + adaptative_params[3]*split(zone_id,".")[2][1:2] + adaptative_params[4]*split(zone_id,".")[2][3:4]
end

function get_adaptative_params(zone_quantity)
    return [zone_quantity*0.1, zone_quantity*0.08, zone_quantity*0.05, zone_quantity*0.012]
end

function get_initial_sequencing(route_zones, params)
    ord_route_zones = Vector{String}()
    zones_values = [get_zone_value(route_zones[i], get_adaptative_params(length(route_zones))) for i in 1:length(route_zones)]
...
function regression_cluster_ordering(ord_training_routes::Vector{Route}, validation_routes::Vector{Route}; params)
    ...
    for R in validation_routes
        ...
        route_city = identify_city(R)
        zones_sequence = get_zones_order(zones(R), clusters_ord[findall(x->x.city_id == route_city, clusters_ord)], dyno_params)
...
function get_dynamic_selection(route, params)
    if(stops_in_region(route.stops,[30.134, 30.517],[-97.951, -97.531]))      # Austin
        insert!(params, 2, 3) # train_zones_segment_offset = dParams[3] {1, 2, 3}
        insert!(params, 9, 4) # train_zones_segment_limit = dParams[4] {7, 9, 10}
        insert!(params, 4, 5) # train_repetition_tolerance = dParams[5] {2, 4, 5}
        insert!(params, 2, 6) # repetition_calculation = dParams[6] {1, 2, 3} = {Multiplication + Sum, Multiplication, Sum}
        insert!(params, 1, 7) # difference_calculation = dParams[7] {1, 2, 3} = {Prod Diff, Mean Diff, Sum Diff}
        insert!(params, 3, 8)# diff_macro_ref = dParams[8] {1, 2, 3}
        insert!(params, 7, 9) # diff_micro_ref = dParams[9] {3, 5, 7}
        insert!(params, 1, 10) # val_zones_segment_offset = dParams[10] {1, 2, 3}
        insert!(params, 7, 11) # val_zones_segment_limit = dParams[11] {7, 9, 10}
        insert!(params, 14, 12) # first_round_compare_val = dParams[12] {12, 14, 18}
        insert!(params, 9, 13) # second_round_compare_val = dParams[13] {7, 9, 10}
        insert!(params, 4, 14) # val_repetition_tolerance = dParams[14] {2, 4, 5}
    elseif(stops_in_region(route.stops,[42.235, 42.397],[-71.199, -70.967]))  # Boston
        insert!(params, 1, 3) # train_zones_segment_offset
        insert!(params, 7, 4) # train_zones_segment_limit
        insert!(params, 2, 5) # train_repetition_tolerance
        insert!(params, 3, 6) # repetition_calculation
        insert!(params, 2, 7) # difference_calculation 
        insert!(params, 3, 8)# diff_macro_ref
        insert!(params, 5, 9) # diff_micro_ref
        insert!(params, 1, 10) # val_zones_segment_offset
        insert!(params, 7, 11) # val_zones_segment_limit
        insert!(params, 12, 12) # first_round_compare_val
        insert!(params, 7, 13) # second_round_compare_val
        insert!(params, 2, 14) # val_repetition_tolerance
    elseif(stops_in_region(route.stops,[41.653, 42.015],[-87.866, -87.487]))  # Chicago
        insert!(params, 2, 3) # train_zones_segment_offset
        insert!(params, 10, 4) # train_zones_segment_limit
        insert!(params, 4, 5) # train_repetition_tolerance
        insert!(params, 3, 6) # repetition_calculation
        insert!(params, 3, 7) # difference_calculation 
        insert!(params, 3, 8)# diff_macro_ref
        insert!(params, 7, 9) # diff_micro_ref
        insert!(params, 3, 10) # val_zones_segment_offset
        insert!(params, 9, 11) # val_zones_segment_limit
        insert!(params, 16, 12) # first_round_compare_val
        insert!(params, 9, 13) # second_round_compare_val
        insert!(params, 4, 14) # val_repetition_tolerance
    elseif(stops_in_region(route.stops,[33.748, 34.312],[-118.639, -118.152]))  # Los Angeles
        insert!(params, 2, 3) # train_zones_segment_offset
        insert!(params, 10, 4) # train_zones_segment_limit
        insert!(params, 3, 5) # train_repetition_tolerance
        insert!(params, 3, 6) # repetition_calculation
        insert!(params, 2, 7) # difference_calculation 
        insert!(params, 3, 8) # diff_macro_ref
        insert!(params, 7, 9) # diff_micro_ref
        insert!(params, 1, 10) # val_zones_segment_offset
        insert!(params, 10, 11) # val_zones_segment_limit
        insert!(params, 14, 12) # first_round_compare_val
        insert!(params, 9, 13) # second_round_compare_val
        insert!(params, 4, 14) # val_repetition_tolerance
    elseif(stops_in_region(route.stops,[47.502, 47.732],[-122.435, -122.244]))  # Seattle
        insert!(params, 2, 3) # train_zones_segment_offset
        insert!(params, 8, 4) # train_zones_segment_limit
        insert!(params, 2, 5) # train_repetition_tolerance
        insert!(params, 3, 6) # repetition_calculation
        insert!(params, 3, 7) # difference_calculation 
        insert!(params, 5, 8) # diff_macro_ref
        insert!(params, 8, 9) # diff_micro_ref
        insert!(params, 2, 10) # val_zones_segment_offset
        insert!(params, 8, 11) # val_zones_segment_limit
        insert!(params, 15, 12) # first_round_compare_val
        insert!(params, 8, 13) # second_round_compare_val
        insert!(params, 3, 14) # val_repetition_tolerance
    else                                                                        # Montevideo
        insert!(params, 1, 3) # train_zones_segment_offset
        insert!(params, 5, 4) # train_zones_segment_limit
        insert!(params, 1, 5) # train_repetition_tolerance
        insert!(params, 3, 6) # repetition_calculation
        insert!(params, 3, 7) # difference_calculation 
        insert!(params, 6, 8) # diff_macro_ref
        insert!(params, 9, 9) # diff_micro_ref
        insert!(params, 2, 10) # val_zones_segment_offset
        insert!(params, 6, 11) # val_zones_segment_limit
        insert!(params, 14, 12) # first_round_compare_val
        insert!(params, 7, 13) # second_round_compare_val
        insert!(params, 2, 14) # val_repetition_tolerance
    end
...
function solve(alg::MinTimeDirectAdaptative, valroute::AbstractRoute, times::TravelTimes, clustord::Vector{ClusterOrdering}; initial_stop=get_station(route), params)
...
function solve(alg::MinTimeDirectAdaptative, route::AbstractRoute, T::AbstractMatrix; zones_list, initial_stop=get_station(route))
    route_id = name(route)
    #T = times.travel_times
    algz = alg.algz
...
function _next_stop(::MinTimeDirectAdaptative, subroute)
...
function make_sequencing_process(alg, routes, data_times)
    ...
    for i in [1:1:length(routes);]
        ...
        #sequences_rand[i] = solve_MinTimeDirectAdaptative(data_route_apply[i], data_times[i].travel_times) #initial_zone = "H24.2C") #get_stop(route,"LZ"))
        # obtener el score
